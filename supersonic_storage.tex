...
{\color{red} dopisaæ o odczycie (nie wiêcej ni¿ strona danych)}
...
przedstawiaj± przepustowo¶æ operacji zapisu w zale¿no¶ci od ustawieñ testu.  O¶
$Z$ zosta³a przesuniêta w celu zwiêkszenia czytelno¶ci.  W przypadku HDD wydaje
siê istnieæ niewielka korelacja miêdzy rozmiarem zapisywanych rodzin a
prêdko¶ci±.
{\color{red} check number of blocks}.

wyra¼nie widaæ, ¿e nie istnieje korelacja miedzy ilo¶ci± zapisywanych rodzin a
prêdko¶ci±. Bardziej istotna jest liczba wierszy przekazywana do pojedynczego
wywo³ania metody \texttt{Write} - ró¿ne warto¶ci powoduj± wahania wydajno¶æ do
oko³o 10\%.\\
...
niezale¿nie od ustawieñ i wynosz± do 10\%. Bior±c pod uwagê drugorzêdn± rolê
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{images/read_throughput_hdd.pdf}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.9\textwidth]{images/read_throughput_ssd.pdf}
...
rozmiaru rodziny w odczytywanym magazynie oraz ¿±danej ilo¶ci wierszy w
pojedynczym wywo³aniu metody \texttt{Next} kursora \texttt{StorageScan}. \\
