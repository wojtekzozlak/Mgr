...
Niestety, Supersonic jest w chwili obecnej wy³±cznie wykonawc± zapytañ i nie
...
\begin{itemize}
  \item{Przewa¿nie operuj± na bardzo du¿ych tabelach - zawieraj±cych wiele
...
u¿ytkownika), a raczej wiêkszych zakresów danych czy wrêcz kompletnych tabel.}
  \item{Czêsto odnosz± siê do stosunkowo niewielkiego podzbioru atrybutów
...
wyników w szerszym kontek¶cie, czê¶æ z testów zosta³a wykonana równie¿ na
...
Maj±c na uwadze specyfikê ¶rodowiska pracy w hurtowni danych, w którym ma
...
Pliki zawieraj±ce relacje o takim samym zbiorze atrybutów powinny byæ ze sob±
...
atrybutów, co mo¿e mieæ bardzo du¿y wp³yw na ilo¶æ danych, które trzeba przes³aæ
...
\begin{itemize}
  \item{W systemie plików systemu operacyjnego, jako jeden lub wiêcej plików.
...
implementacji i mo¿na przyk³adowo wykorzystywaæ wierszowo zorientowane strony
...
obrêbie strony. Ka¿dy rekord posiada nag³ówek zawieraj±cy maskê bitow±
wskazuj±c± na istnieniu danych (przy obs³udze warto¶ci \textit{NULL}), pozycje
atrybutów zmiennej d³ugo¶ci oraz dane zwi±zane z implementacj±. Ze wzglêdu na
zmienn± d³ugo¶æ rekordów na koñcu strony utrzymywany jest indeks, utrzymuj±cy
pozycje kolejnych wpisów w obrêbie strony. \\
...
momencie, gdy interesuj± nas tylko niektóre atrybuty z ca³ego wiersza tabeli,
mo¿e siê okazaæ, ¿e interesuj±cych danych mie¶ci siê bardzo ma³o i trzeba czêsto
...
sprawiaj± równie¿ przeszukiwania za pomoc± indeksów, które dokonuj± du¿ej liczby
...
danych oraz wiêksz± liczb± przesuniêæ g³owicy potrzebnych do rekonstrukcji
...
w konsekwencji marnuje siê pasmo medium.
...
tego PAX wydaje siê byæ najbardziej przysz³o¶ciowym schematem organizacji danych
spo¶ród tu przedstawionych.


\section{Kompresja danych}

Zagadanienie wykorzystania kompresji w systemach bazodanowych jest bardzo
obszerne i mog³oby stanowiæ g³ówny temat oddzielnego opracowania. Omawiane tej w
...
a urz±dzeniem. Dodatkowo, w przypadku systemów rozproszonych, pozwala to na 
...
\begin{itemize}
  ...
  \item{\textbf{algorytmy lekkie} - przydatne w bardzo specyficznych
      ...
      dostêp do danych, bez potrzeby dekompresji wiêkszego kontekstu, a tak¿e
...
jako pary \textit{(element, liczba)}. Przyk³adowo, sekwencja znaków \texttt{a w
w z z z z z d d d e f f} reprezentuje sekwencja par \texttt{(a,1) (w,2) (z,5)
(d,3) (e,1) (f,2)}. W zale¿no¶ci od rozk³adu elementów uzyskaæ mo¿na ró¿n±
efektywno¶æ kompresji (tak¿e negatywn±), w zwi±zku z czym nie mo¿na jej stosowaæ
w ka¿dym przypadku.  Warto zauwa¿yæ, ¿e wiele operacji (np. arytmetycznych)
mo¿na wykonywaæ bezpo¶rednio na elementach bez potrzeby rekonstrukcji
pierwotnych danych.
...
Dla danego bloku numerycznych warto¶ci znajduje najmniejsz± warto¶æ $min$, a
nastêpnie zapisuje ka¿d± warto¶æ $v[i]$ jako $v[i] - min$ korzystaj±c z
\textit{Prefix-Suppresion}. Schemat ten dobrze stosuje siê sklastrowanych do
...
zapytania. Podobny problem jest z indeksami, które czêsto pozwalaj± bardzo
...
zapytania tak, by obliczenie odbywa³o siê w bardziej efektywny sposób. 
...
W celu rozwik³ania tego problemu zastosowano rozwi±zanie polegaj±ce na
...
przez opracowan± implementacjê. Id±c krok dalej mo¿na pokusiæ siê o oddzielenie
...
rozmiar\_strony)}.  Umo¿liwia on swobodny dostêp bezpo¶rednio do ¿±danych
...
¿e w skrajnych przypadkach dane dotycz±ce wiersza mog± znajdowaæ siê na dwóch,
przeciwnych koñcach pliku magazynu. Jednak¿e, dobieraj±c odpowiednio rozmiar
strony oraz podzia³ schematu na atrybuty, mo¿na zachowaæ dostatecznie dobr±
lokalno¶æ danych.

\subsubsection{Metadane}

Przy zapisie stron danych dla relacji gromadzone s± nastêpuj±ce metadane:
\begin{itemize}
...
uwagi - niech umownie ma numer $0$), nie powi±zana z ¿adn± rodzin± atrybutów. W
...
dostarczaj±c kolejne porcje danych o ograniczonej liczbie wierszy w celu ich
...
mo¿liwe jest zlecenie odczytu wy³±cznie zadanego podzbioru atrybutów relacji.
...
zadanego wiersza. Maj±c wiedzê o liczbie rekordów w ka¿dej stronie mo¿na z
...
\begin{itemize}
  \item{Schemat tabeli dzielony jest na rodziny za pomoc± strategii
...
reprezentuje uj¶cia danych. Udostêpnia on metodê \texttt{Write}, która przyjmuje
...
przekroczy zadany limit, zostaje ona zapisana do strumienia reprezentowanego
...
u¿yciu metody \texttt{Finalize}. W efekcie:
  ...
  uszkodzonych plików magazynu, nie nadaj±cych siê do odczytu.}
  \end{itemize}
}
\end{itemize}
...
odpowiednich stron. Ze wzglêdu na to, ¿e \texttt{PageReader} implementuje
interfejs kursora, mo¿na go traktowaæ jak pe³nowarto¶ciow± operacjê odczytu
...
odpowiada z³±czeniu tablic przy u¿yciu sztucznie wprowadzonego atrybutu
...
zadanej przez u¿ytkownika strategii, reprezentowanej przez interfejs
...
grupy zawieraj±ce narzucon± z góry liczbê atrybutów (z wyj±tkiem ostatniej).  W
...
mieæ istotny wp³yw na efektywno¶æ magazynu. Wczytuj±c dane mo¿emy pobraæ strony
reprezentuj±ce tylko wybrane rodziny atrybutów. Nie da siê jednak dostaæ do
...
buforów danych o ró¿nych d³ugo¶ciach. Sk³ada siê z nastêpuj±cych czê¶ci:
\begin{itemize}
  \item{\textbf{nag³ówek strony} - zawieraj±cy informacje o jej rozmiarze oraz
liczbie przechowywanych buforów,}
  \item{\textbf{tablicê wska¼ników na bufory} - okre¶laj±ca pozycje
poszczególnych buforów w obrêbie strony,}
  \item{\textbf{nag³ówki buforów} - zawieraj±ce informacje o rozmiarze buforu; w
...
sposóbi, w jaki przechowywany jest indeks. Pierwsze cztery bajty zajmuje liczba
...
zawarto¶æ. Przestrzeñ alokowana jest od pocz±tku bloku areny, natomiast od koñca
...
\textit{ProtocolBuffer}\cite{protobuf}, opublikowana przez firmê Google na
...
\textbf{rodzinie stron} o numerze \textbf{0}. Jest to specjalna rodzina stron,
...
zaimplementowano klasê \texttt{EnumeratedFileSeries}, generuj±c± nazwy na
...
natomiast wiêzy dotycz±ce podzia³u schematu na rodziny - mo¿e on byæ zupe³nie 
...
systemu plikówm, zgodnych ze standardem POSIX. \\
...
magazyn danych, przystosowane s± do pracy w ¶rodowisku, w którym nie modyfikuje
...
testu porównawczego \textit{TPC-H} (w wersji 2.17.0). Sk³ada siê ona z 16 kolumn
o typach liczbowych i znakowych. Ze wzglêdu na brak dok³adnych odpowiedników w
...
Przy generowaniu danych za pomoc± narzêdzi udostêpnianych przez \textit{TPC-H}
u¿yto wspó³czynnika skalowania $s = 4$. W efekcie uzyskano relacjê zawieraj±c±
23'996'604 rekordów, zajmuj±cych w formie tekstowej 2942\ MB przestrzeni
dyskowej.
...
atrybutów;}
  \item{liczby wierszy przekazywanej do pojedynczego wywo³ania
...
W przypadku SSD w zasadzie brak jakiejkolwiek regularno¶ci w wynikach, z
wyj±tkiem faktu, ¿e wszystkie oscyluj± w okolicach 180 MB/s. Wyniki
pojedynczych testów cechowa³a spora wariancja (20-30 MB/s). Byæ mo¿e da³oby siê
...
Zachowanie to wynika najprawdpodobniej z uk³adu stron danych w obrêbie
...
co znacznie poprawia efektywno¶æ. Z reszt± nie jest ju¿ tak ³atwo - co prawda
magazyn z pe³nym podzia³em wymaga mniejszej ilo¶ci przesuniêæ ni¿ 3 pozosta³e,
jednak przypadek podzia³u na rodziny rozmiaru 8 wymaga ich najwiêcej wcale nie
bêd±c najwolniejszym! \\
...
schodkowe. Rozmiar stopnia zale¿y za¶ od wzglêdnego rozmiaru strony, co w tym
konkretnym przypadku testowym oznacza dok³adno¶æ pominiêcia rekordów na poziomie
kilku promili.
...
ca³ymi rodzinami. Oszczêdno¶æ czasu jest proporcjonalna do liczby rodzin, które
mo¿na pomin±æ.
...
sytuacji, przez co nie uda³o siê uzyskaæ ¿adnych miarodajnych wyników.
...
porównania z wynikami testu opisywanego magazynu. \\
...
wykonania jest co prawda skorelowany z ilo¶ci± atrybutów, lecz ró¿nice s±
bardzo ma³e. Sugeruje to, ¿e istniej± one na poziomie przetwarzania danych
przez procesor, a nie komunikacji ze sk³adem danych. Omawiany w pracy magazyn w
...
\texttt{L\_LINENUMBER}), w którym fizycznie posortowano rekordy (w bazie
...
Gdzie \texttt{X} by³ uprzednio wyliczonym numerem gwarantuj±cym odpowiedni±
...
stosunku do formatu tekstowego uzyskuje on przewagê dziêki kolumnom liczbowym i
...
wymaga to rezerwacji dostatecznej ilo¶ci miejsca, aby ewentualna zmiana nie
wymaga³a relokacji rekordu. W definicjach wygenerowanych przez \texttt{TPC-H}
...
W dzisiejszych czasach problem efektywnego przetwarzania ogromnych zbiorów
...
warunkach du¿o lepiej sprawdzaj± siê systemy kolumnowe, które mimo istnienia
od przesz³o trzydziestu lat, nigdy nie uzyska³y szerokiego audytorium. \\
...
Niestety, trudno go w chwili obecnej nazwaæ pe³noprawnym systemem bazodanowym -
