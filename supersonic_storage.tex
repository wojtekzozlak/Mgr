...
\section{Supersonic jako kolumnowy silnik zapytañ}
...
Supersonic\cite{supersonic} jest wysokowydajnym, kolumnowym silnikiem zapytañ
...
\section{¦rodowisko hurtownii danych}
...
zbadanie zachowania sk³adu danych dla silnika Supersonic przystosowanego do
pracy w opisanym wcze¶niej ¶rodowisku. \\
...
\subsection{Sposób ewaluacji rozwi±zania}
{\color{red} czê¶ciowo przenie¶æ do Zakresu, czê¶ciowo do ewaluacji, a potem
usun±æ.} \\
...
\begin{itemize}
  \item{Przepustowo¶æ operacji zapisu i odczytu w megabajtach na sekundê.}
  \item{Zale¿no¶æ miêdzy liczb± kolumn wymaganych w zapytaniu a czasem
odczytu danych.}
...
poziomie systemu operacyjnego, okre¶laj±c± najmniejsz± jednostkê danych, które
...
\begin{itemize}
  \item{liczb± operacji wej¶cia/wyj¶cia niezbêdnych do realizacji zapytania,}
...
udzia³ tylko niewielki podzbiór kolumn. Strony danych odczytywane s± zawsze w
ca³o¶ci, co w przypadku NSM oznacza ci±gi kompletnych rekordów. W momencie gdy
zapytanie dotyczy wy³±cznie u³amka atrybutów wiêkszo¶æ pobranych danych jest
zupe³nie nieprzydatna. Nieefektywne wykorzystanie magazynu i marnowanie operacji
wej¶cia / wyj¶cia dramatycznie pogarsza wydajno¶æ systemu bazodanowego. \\ 

{\color{red} add sth regarding cache}
...
odwo³uj±cymi siê do ma³ej liczby (a czêsto nawet pojedynczych) rekordów.
...
idzie, redukcjê liczb± operacji wej¶cia/wyj¶cia niezbêdnych do przeniesienia
...
liczby atrybutów bior±cych czynny udzia³ w obliczeniu wymagane jest wczytanie
...
Umo¿liwia ona swobodny dostêp bezpo¶rednio do ¿±danych stron, bez potrzeby
przegl±dania ca³ego magazynu. \\

{\color{red} dodaæ, ¿e trzymamy te¿ rozmiar, ¿eby oszczêdziæ operacje seek}
...
implementuj±cej interfejs \texttt{Sink} silnika \textit{Supersonic}, który
...
numer, liczba stron oraz ich kolejne pozycje w obrêbie pliku (strony zawsze maj±
ci±g³± numeracjê, pocz±wszy od zera). \\
...
8\ GB).}
  \item{\textbf{Dyski}:
    \begin{itemize}
      \item{\textbf{SSD} - Samsung 840 EVO 256\ GB SATA3 (przechowuj±cy równie¿ system
operacyjny),}
...
magazynu o rozmiarze oko³o 128\ MB, zawieraj±cych strony danych o rozmiarze oko³o
...
korelacja miêdzy rozmiarem zapisywanych rodzin a prêdko¶ci±. Mo¿na podejrzewaæ,
¿e jest to zwi±zane z liczb± stron w magazynie jednak na podstawie rysunku
\ref{figure:data_pages} trudno stwierdziæ wyra¼n± zale¿no¶æ. Bardziej istotna
wydaje liczba wierszy przekazywana do pojedynczego wywo³ania metody
\texttt{Write} - ró¿ne warto¶ci powoduj± wahania wydajno¶ci w granicach 10\%.\\
...
je bardziej ustabilizowaæ, przez u¿ycie wiêkszego zbioru danych. Poniewa¿ dyski
SSD s± wra¿liwe na wielokrotny zapis du¿ych ilo¶ci danych i wi±za³oby siê to z
ryzykiem uszkodzenia no¶nika, w zwi±zku z czym autor nie podj±³ takiej próby. \\

W obu przypadkach wahania przepustowo¶ci operacji zapisu nie s± zbyt du¿e,
niezale¿nie od ustawieñ i wynosz± 10-15\%. Bior±c pod uwagê drugorzêdn± rolê
operacji zapisu w funkcjonowaniu projektowanego sk³adu jest to ró¿nica ma³o
istotna. Sama prêdko¶æ w obu przypadkach wynosi 65-75\% maksymalnej prêdko¶ci
zapisu sekwencyjnego no¶ników (oko³o 280 MB/s w przypadku SSD, 80 MB/s HDD).
Nie jest to mo¿e wynik osza³amiaj±cy, ale w tym przypadku ca³kowicie
satysfakcjonuj±cy. \\
...
przypadkach obserwujemy bardzo podobne zachowanie - drobne wahania przy
rozmiarach rodzin od 1 do 8 oraz gwa³towny skok wydajno¶ci o 30-40\% przy
zapisie wszystkich danych razem. W wypadku pojedynczej rodziny atrybutów mamy de
facto do czynienia z sekwencyjnym odczytem stron z medium. Pozwala to na
wykorzystanie pe³ni jego mo¿liwo¶ci. \\

{\color{red} dodaæ wykres z ilo¶ci± seeków}
Brak zale¿no¶ci miêdzy rozmiarem ¿±dania i przepustowo¶ci± wynika z semantyki
operacji odczytu.

Brak zale¿no¶ci miêdzy rozmiarem ¿±dania a przepustowo¶ci± odczytu pokazuje, ¿e
koszt ca³ej operacji jest zdominowany przez pobieranie danych ze stron oraz ich
deserializacjê. 
...
\textit{MySQL} oraz \textit{PostgreSQL}. Na obu wykonano testy analogiczne do
opisanych w sekcjach \ref{sec:read_offset} oraz \ref{sec:read_subset}. Niestety,
systemy te nie udostêpniaj± prostej metody bezpo¶redniego dostêpu do magazynu
danych. Z tego powodu, w trakcie testu wykonywane by³y zapytania postaci: \\
...
\subsubsection{\color{red}porównanie z monetdb i infobright}

Dlaczego siê nie uda³o
...
\bibitem{snappy} TODO: https://github.com/google/snappy

\bibitem{lz4} TODO: https://code.google.com/p/lz4/
\end{thebibliography}
